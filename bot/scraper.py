...
class WebScraper:
    ...
    def __init__(self) -> None:
        ...
        self.json_file = os.path.join(self.__db_path, JSON_FILE_NAME)
...
class NekretnineScraper(WebScraper):
    ...
    def scrape_page(self, soup) -> list:
        ...
        # Only links:
        # offer_urls_elements = soup.findAll(self.data["LINK_HREF_OFFER_TAG"], class_=self.data["LINK_HREF_OFFER_CLASS"])
        
        # for offer_url_element in offer_urls_elements:
        #     offer_url_cleaned = self.data["BASE_URL"] + offer_url_element.find('a').get('href')
        #     offers.append(offer_url_cleaned)

        print("start:")
        start = time.perf_counter()
        offer_elements = soup.findAll(self.data["OFFER_TAG"], class_=self.data["OFFER_CLASS"])
        end = time.perf_counter()
        print(f"Time offer_elements: {end - start:0.4f} seconds")
        print(f"Length offer_elements: {len(offer_elements)} elements")
        print(f"Size offer_elements: { sys.getsizeof(offer_elements) / 1000 } Kbytes")

        count = 0
        for offer_element in offer_elements:
            print(f"{count}/{QUANTITY_OFFERS}:")
            ...
            print(f"Time offer_element { count }: {end - start:0.4f} seconds")
            print(f"Length offer_element { count }: {len(offer_element)} elements")
